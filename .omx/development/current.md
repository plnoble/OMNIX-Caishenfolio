...
- Status: **R0~R5 + V1~V4 + A1~A4 + S1~S2 + F1~F5 + G1~G4 + H1~H4 完成 (v0.13.1 / R5)**
- 应用内更新已启用：下载 → SHA-256 校验 → ECDSA P-256 验签 → 安装；任一层不过即删除文件
...
```powershell
dotnet build Caishenfolio.slnx; if ($?) { dotnet test Caishenfolio.slnx }   # 362 通过 / 0 警告
